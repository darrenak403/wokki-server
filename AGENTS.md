...
- **Branch workspace access is scoped** — Admin manages every branch. Manager manages only locations assigned through `LocationManager`; never infer Manager scope from role alone or from the Manager's own employee/department memberships. Employees without an Active `LocationMembership` stay out of protected app routes: no membership → `/join`, Pending/Rejected/Left/Transferred → `/pending`; join/review is by Admin or the target branch's Manager.
