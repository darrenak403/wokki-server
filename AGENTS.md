...
- **Attendance verification is advisory and non-blocking** — `POST` clock-in optionally accepts `Latitude`, `Longitude`, `PhotoBase64`, `PhotoContentType`, `FaceEmbeddingJson`, `FaceMatch` (all optional, never required). Server computes three advisory flags: `GpsOutOfRange` (Haversine vs. configurable geofence radius, default 150m), `IpMismatch` (client IP vs. `Location.NetworkIpOrCidr` CIDR match), `FaceMismatch` (client-reported boolean only, no server-side ML); first photo per employee enrolls face, no flag computed that record; subsequent photos compare client `FaceMatch` to enrollment. Flags + derived `IsFlagged` returned on attendance list/audit but never block check-in. Photos stored via Cloudinary signed delivery; check-in selfies auto-purged after configurable window (default 30 days), enrollment photos exempt. Background `AttendancePhotoCleanupWorker` handles purge. Manager/Admin see flags for their scope only. `EmployeeResponse` includes `HasFaceEnrollment` boolean. Employee can fetch their stored face descriptor via `GET /api/v1/self/face-descriptor` to support client-side face comparison on subsequent check-ins. `CreateLocationRequest` and `UpdateLocationRequest` now accept optional `Latitude`, `Longitude`, `NetworkIpOrCidr` fields (with range validation: lat ±90, lng ±180, CIDR max 64 chars) to allow Admins to configure geofence and network boundaries.
