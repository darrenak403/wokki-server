...
Wokki Shift Ops is an internal workforce operations platform for a single enterprise per deployment. It replaces fragmented tools for **weekly scheduling**, **shift swaps**, **time tracking**, **payroll preparation**, and **team messaging**.
...
- **Single-tenant per instance**: one company, one database, one API deployment.
- New customers are onboarded by **provisioning a new instance**, not self-service signup in a shared SaaS pool.
- MVP targets **internal pilot** at one enterprise.
...
1. **Access** — role matrix; User isolated to self-service paths.
