...
| FR-105 | P1       | Org Admin creates staff through the employee workflow with a `DepartmentId`; the system creates both User account and Employee profile, then auto-provisions Active `LocationMembership` at that department's location. Same-org legacy Users without Employee can be linked through this workflow. Employees log in directly — no standalone org staff accounts, self-serve join request, or `/join` gate. |
