...
| BR-072 | Auto-scheduling is scoped to one department/week and requires a saved location scheduling policy, active department employees, active shifts, and submitted preferences when the branch rule requires it. Branch policy (`location-scheduling-policy.v3`) includes seven core F&B rules read by the scheduler: `max_shifts_per_week`, `max_shifts_per_day`, `require_role_match`, `require_submitted_preferences`, `unavailable_is_hard_block` (hard block or soft penalty), `preferred_weight` (score bonus for "Preferred" preference, default 30), `available_weight` (score bonus for "Available" preference, default 5). When `unavailable_is_hard_block=false`, employees with "Unavailable" preference receive a score penalty of `-preferred_weight` instead of being hard-blocked. Plus optional custom branch rules Admin/Manager may add. Missing input returns explicit `reason` such as `missing_location_rules`, `no_employees`, `no_shifts`, or `missing_preferences`. | `HeuristicScheduleSuggestionService`, `LocationSchedulingPolicyRules`, `SchedulingAssignmentRules` |
