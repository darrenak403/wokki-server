...
| BR-094 | **Org Admin** may transition staff between **`User` ↔ `Manager`** via `POST /api/v1/employees/{employeeId}/role-transition` (atomic: `User.Role`, `LocationManager`, `Employee.DepartmentId`, department memberships, `LocationMembership`, `AuditLog`, refresh-token revoke). **Promote:** `targetRole=Manager`; optional `locationId` (defaults from current department's branch); clears department memberships; ends active `LocationMembership` as `Transferred`; creates `LocationManager`. **Demote:** `targetRole=User`; requires `departmentId`; removes all org `LocationManager` rows; restores department + active `LocationMembership`. **Manager** may not call role-transition (403); department-only moves stay on `POST /api/v1/department-memberships/transfer`. Demoting the last branch Manager is allowed — Org Admin retains full branch scope. Legacy `PATCH /api/v1/users/{id}/role` must not change role silently (400 → use role-transition). | `EmployeeRoleTransitionService`, `StaffPlacementCoordinator`, `EmployeeEndpoints` |
...
| BR-098 | **Attendance verification is advisory and non-blocking** — `POST /clock-in` optionally accepts `Latitude`, `Longitude`, `PhotoBase64`, `PhotoContentType`, `FaceEmbeddingJson`, `FaceMatch` (all optional; no validation requirement). Server computes three advisory flags on every clock-in: (a) `GpsOutOfRange` — Haversine distance vs. `Location.Latitude`/`Longitude` and configurable geofence radius (`AttendanceVerification:DefaultGeofenceRadiusMeters`, default 150m); (b) `IpMismatch` — authenticated user's real IP (from `ICurrentUserService.IpAddress`, resolved via `UseForwardedHeaders`) vs. `Location.NetworkIpOrCidr` (CIDR match); (c) `FaceMismatch` — boolean reported by client only (no server-side ML/face comparison by design). **First-ever clock-in with photo for an employee becomes face enrollment: selfie + face-api.js descriptor saved to `Employee.FaceEnrollmentPhotoUrl`/`FaceEmbedding`; no `FaceMismatch` computed for enrollment record.** Subsequent clock-ins report `FaceMatch` from client, recorded as `FaceMismatch = !FaceMatch`. Verification flags + derived `IsFlagged` returned on `GET /attendance` and audit endpoints (Manager/Admin for their scope); flags **never block** clock-in. Photos (check-in selfie + enrollment) uploaded via Cloudinary authenticated/signed delivery (not public unsigned). Background worker `AttendancePhotoCleanupWorker` auto-purges check-in selfies older than configurable retention (`AttendanceVerification:PhotoRetentionDays`, default 30 days); face-enrollment reference photo structurally excluded from purge. Bedrock scheduling does not depend on verification flags. **Employee self-endpoints:** `GET /api/v1/self/face-descriptor` returns the authenticated employee's stored `FaceEmbeddingJson` (string | null) for client-side face comparison; `EmployeeResponse` includes `HasFaceEnrollment` boolean. **Location geofence/network config:** `POST /api/v1/locations` and `PUT /api/v1/locations/{id}` now accept optional `Latitude` (±90), `Longitude` (±180), `NetworkIpOrCidr` (max 64 chars); validators enforce ranges when fields are provided. | `AttendanceService.ApplyVerificationAsync`, `AttendancePhotoCleanupWorker`, `AttendanceResponse`, `CloudinaryPhotoUploadService`, `EmployeeSelfEndpoints.GetMyFaceDescriptorAsync`, `EmployeeResponse`, `CreateLocationRequest`, `UpdateLocationRequest` |
...
| BR-097 | **Scheduling hints** (ephemeral, per-call only): Manager may confirm a structured hint from the schedule insight assistant that temporarily shapes exactly one upcoming `suggest` call via an optional `Hint` field in `SuggestScheduleRequest`. **Three allow-listed hint shapes:** (a) PreferenceWeight—boost/reduce scoring for a role/department or explicit employee list; (b) TempMinMax—override total-assignment-count bounds for a group/person for one call only; (c) AvoidPairing—prevent two named employees from sharing the same shift+date in one call. A hint is **never** persisted to `OrganizationSchedulingPolicy` or any policy table; it affects only the immediate `suggest` call it decorates. If a hint is invalid (fails roster validation) the response returns `Reason: "invalid_hint"`; if infeasible, returns `Reason: "hint_infeasible"` (distinct from generic `"infeasible"`). `apply-suggestions` has **no hint field** by design — hints never gain a write path. The `/schedules/{id}/insights/chat` endpoint may generate hints via an explicit `HintMode` toggle (Bedrock advisory only; chat never writes assignments). | `ScheduleInsightService.ChatAsync`, `ScheduleSuggestionHintValidator`, `CpSatScheduleSuggestionService.SolveAsync` |
