...
| BR-086 | CP-SAT suggest inputs are loaded by `ScheduleSuggestionContextLoader` (org policy, dept employees, active shifts, **Submitted** preferences, existing assignments, availabilities, 4-week history). `POST .../suggest` returns suggestions only; context snapshot auto-refreshes after suggest. **No auto-apply** and **no auto-rebalance** when preferences or approved leave change after assignments exist — Admin sees rebalance hints on `GET /schedules/{id}` and uses the same **Tạo gợi ý AI** button (no separate re-solve action). | `ScheduleRebalanceAnalyzer`, `ScheduleService`, `CpSatScheduleSuggestionService` |
