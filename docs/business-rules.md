...
| BR-072 | Auto-scheduling is scoped to one department/week and requires active department employees, active shifts, and submitted preferences when the org enforced rule requires it. **Org policy** (`org-scheduling-policy.v1`) is shared across all branches; platform catalog via `GET /api/v1/scheduling/rule-catalog`; Org Admin `GET/PUT /api/v1/org/scheduling-policy` (Manager read-only). Exposes **4 enforced solver rules** for SMB F&B: preference preflight/hard block, default min staff per shift, `require_role_match`. Coverage fill, rest between shifts, weekly min/overtime caps use `SchedulingSolverDefaults` (not org UI). Up to **20 advisory** custom rules (stored, not read by solver). Missing org row uses catalog defaults. Apply gợi ý is always explicit (review UI + Apply button). Typed mapping: `OrganizationSchedulingSolverPolicy`. Missing input returns explicit `reason`. | `CpSatScheduleSuggestionService`, `OrganizationSchedulingPolicyRules`, `OrganizationSchedulingSolverPolicy`, `SchedulingRuleCatalog`, `SchedulingSolverDefaults` |
