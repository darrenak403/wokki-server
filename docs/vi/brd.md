...
| **Wokki Platform Operator** | Admin Wokki | Giám sát platform, kích hoạt/gia hạn gói org | `/platform`, platform stats, list user/org toàn hệ thống, bật/tắt gói org |
...
| FR-105 | P1 | Org Admin tạo staff qua workflow nhân viên kèm `DepartmentId`; hệ thống tạo cả tài khoản User và hồ sơ Employee, rồi tự tạo Active `LocationMembership` tại chi nhánh phòng ban. User legacy cùng org chưa có Employee có thể được liên kết qua workflow này. Nhân viên đăng nhập trực tiếp — không có tài khoản staff trần, không có `/join` hay duyệt yêu cầu. |
