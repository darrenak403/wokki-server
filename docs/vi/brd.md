...
| OBJ-06 | Hỗ trợ manager gợi ý phân ca | Heuristic suggest/apply trên lịch Draft |
...
| **Nhân viên (User)** | Vận hành tuyến đầu | Xem ca, đổi ca, chấm công | `/me/*`, swap, clock |
...
| 2 | Mẫu ca, lịch tuần, publish/unpublish, copy tuần, `/me/schedule` |
...
| 5 | Chat (REST + SignalR), gợi ý lịch heuristic |
...
| LLM ngoài cho lịch | Chỉ heuristic; LLM sau qua cùng interface |
...
| FR-205 | P1 | Nhân viên xem ca sắp tới qua `/me/schedule`. |
...
3. Publish → thông báo nhân viên → hiện trên `/me/schedule`.
...
| Gợi ý AI | Heuristic | LLM sau `IScheduleSuggestionService` |
