...
| BR-004 | `Manager` quản lý lịch, phân ca, ghi đè đổi ca, điều chỉnh chấm công, mở **tin nhắn riêng** với nhân viên trong org (cùng quy tắc các role khác). | Authorization route |
...
| BR-008 | Org Admin tạo staff qua `POST /api/v1/employees`; một workflow tạo cả `User` và `Employee`. **Role `User` bắt buộc `DepartmentId`** (chi nhánh + phòng ban hiện tại) và tự tạo **Active** `LocationMembership` tại chi nhánh của phòng ban đó. **Role `Manager` bắt buộc `LocationIds`** (ít nhất một chi nhánh) — hệ thống tạo `LocationManager` lúc tạo; Manager không cần phòng ban. Khi đăng nhập, **User** và **Manager** chỉ thấy chi nhánh họ có quyền truy cập (`LocationMembership` / `LocationManager`); chi nhánh không được gán sẽ không hiện. Nếu đã có `User` legacy cùng org nhưng chưa có `Employee`, workflow này sẽ liên kết user đó thành hồ sơ nhân viên. Không tạo tài khoản org staff trần qua `/users`. Sau onboarding, nhân viên đăng nhập vào `/app` trực tiếp. | `EmployeeService.CreateAsync`, `EmployeeService.AssignManagerLocationsAsync`, `EmployeeService.EnsureActiveLocationMembershipAsync` |
| BR-020 | **Tự onboard nhân viên (song song BR-008):** `POST /auth/register-employee` → `User` không org, họ tên trên `users`. `GET /organizations/directory` (org có gói active). Một `OrgJoinRequest` Pending toàn hệ thống. Admin duyệt (`departmentId`, `hourlyRate`) hoặc từ chối. Manager không tự onboard. Hủy request: `DELETE /org-join-requests/me`. Hết gói org → request Expired. | `AuthService.RegisterEmployeeAsync`, `OrgJoinRequestService`, `IEmployeeProvisioner` |
...
| BR-094 | **Org Admin** chuyển **User ↔ Manager** qua `POST /api/v1/employees/{employeeId}/role-transition` (một transaction: role, `LocationManager`, phòng ban, membership, audit, revoke token). **Promote:** `Manager` + `locationId` (tùy chọn, mặc định từ phòng ban hiện tại). **Demote:** `User` + bắt buộc `departmentId`; xóa mọi `LocationManager`. **Manager** không gọi API này (403). Demote Manager cuối chi nhánh vẫn được phép. `PATCH /users/{id}/role` không đổi role âm thầm (400). | `EmployeeRoleTransitionService`, `EmployeeEndpoints` |
...
| BR-016 | `POST /register` tạo atomic `Organization` + Org Admin (`Admin`) + **hồ sơ `Employee` liên kết** (thành viên kênh org) + JWT. Org Admin **`Employee` không có `DepartmentId`** — phạm vi toàn org/mọi chi nhánh; chat/UI hiển thị **Quản trị viên · Toàn chi nhánh**. **Không hạ role Admin / gán phòng ban cho org creator qua `POST /employees`**; danh sách chat tự sửa org creator bị gán nhầm (role → Admin, xóa phòng ban). Một email = một org; email trùng → 409. Org mới mặc định chưa có gói cho tới khi Wokki admin kích hoạt. Admin org cũ chưa có Employee được tự tạo khi vào chat/self-profile lần đầu. | `AuthService.RegisterAsync`, `OrgAdminEmployeeProvisioner`, `ChannelService.ListOrgMembersAsync` |
...
| BR-090 | Mọi cập nhật gói qua platform phải tạo `OrganizationSubscriptionLedgerEntry` bất biến và một `AuditLog` trong cùng transaction với thay đổi gói org. Ghi ledger là bắt buộc; nếu ledger/audit lỗi thì cập nhật gói lỗi theo. Ledger chỉ lưu lịch sử gói (trạng thái, thời hạn, ngày hết hạn, operator, timestamp), không lưu doanh thu, hóa đơn hoặc dữ liệu thanh toán. | `PlatformAdminService`, `OrganizationSubscriptionLedgerRepository`, `AuditLog` |
| BR-091 | Support Console phase 1 chỉ đọc với dữ liệu nghiệp vụ tenant. Nó được tìm theo org id, tên org hoặc email user và trả trạng thái gói, số lượng, timestamp vận hành mới nhất, ledger mới nhất; không được impersonate user, không lộ nội dung dòng nghiệp vụ tenant, không sửa lịch/lương/chấm công/chat, không thao tác destructive trên tenant. Write path hỗ trợ duy nhất là action subscription sẵn có ở phạm vi platform. | `PlatformAdminService`, `OrganizationRepository.SearchPlatformSupportAsync`, `GetPlatformSupportContextAsync` |
| BR-092 | Usage analytics định nghĩa active org là org có ít nhất một `PlatformActivityEvent` trong window: login thành công, publish lịch, suggest lịch, apply-suggestions, clock-in, clock-out hoặc gửi tin chat. Ghi activity là best-effort, không lưu payload body/request, và không rollback workflow org chính. | `PlatformActivityRecorder`, `PlatformUsageAnalyticsService` |
| BR-093 | Platform diagnostics chỉ dành cho `PlatformOperator` và tách khỏi public `/health`. Platform health có thể trả trạng thái API, Bedrock, email config/failure state nhưng không lộ SMTP recipient, credential, token hoặc dữ liệu tenant. Lỗi dependency làm degrade response theo component, không làm hỏng toàn endpoint. | `PlatformDiagnosticsService`, `PlatformDiagnosticState`, `EmailDiagnosticService` |
...
| BR-041 | Clock-in cần ca **published** hôm nay. | `AttendanceService` |
...
| BR-098 | **Xác minh chấm công chỉ mang tính tham khảo và không chặn** — `POST /clock-in` tùy chọn chấp nhận `Latitude`, `Longitude`, `PhotoBase64`, `PhotoContentType`, `FaceEmbeddingJson`, `FaceMatch` (tất cả không bắt buộc; không validate bắt buộc). Server tính toán ba cờ tham khảo: (a) `GpsOutOfRange` — khoảng cách Haversine so với `Location.Latitude`/`Longitude` và bán kính geofence tùy chỉnh (`AttendanceVerification:DefaultGeofenceRadiusMeters`, mặc định 150m); (b) `IpMismatch` — IP thực của user đã xác thực (từ `ICurrentUserService.IpAddress`, qua `UseForwardedHeaders`) so với `Location.NetworkIpOrCidr` (CIDR match); (c) `FaceMismatch` — boolean do client báo cáo (không ML server-side, thiết kế như vậy). **Lần đầu clock-in có photo cho một nhân viên là lúc đăng ký khuôn mặt: ảnh selfie + descriptor face-api.js lưu `Employee.FaceEnrollmentPhotoUrl`/`FaceEmbedding`; không tính `FaceMismatch` cho bản ghi đăng ký này.** Clock-in kế tiếp client báo `FaceMatch`, ghi thành `FaceMismatch = !FaceMatch`. Cờ xác minh + `IsFlagged` dẫn xuất trả trên `GET /attendance` và audit endpoint (Manager/Admin phạm vi của họ); cờ **không bao giờ chặn** clock-in. Ảnh (selfie check-in + đăng ký) upload qua Cloudinary xác thực/signed delivery (không public unsigned). Worker `AttendancePhotoCleanupWorker` tự động xóa selfie check-in cũ hơn khoảng thời gian lưu (`AttendanceVerification:PhotoRetentionDays`, mặc định 30 ngày); ảnh đăng ký khuôn mặt không bị xóa do cấu trúc riêng. Bedrock lịch không dùng cờ xác minh. | `AttendanceService.ApplyVerificationAsync`, `AttendancePhotoCleanupWorker`, `AttendanceResponse`, `CloudinaryPhotoUploadService` |
...
| BR-055 | Chỉ chấm công **theo ca** có `ClockOut` mới tính lương. | `SumWorkedMinutesByEmployeeAsync` |
| BR-056 | OT = phút OT duyệt × cùng `HourlyRate`. | `PayrollCalculationService` |
| BR-057 | Lock tạo snapshot `PayrollLine` bất biến. | `LockPeriodAsync` |
| BR-058 | Kỳ lương chính = **tháng dương lịch** / phòng ban. | FE + API |
| BR-059 | Cờ **đã chuyển lương** trên line đã lock; Admin only. | `SetLinePaidAsync` |
...
| BR-063 | Không cho tạo kênh **Group** tùy ý (`403`); bản ghi Group cũ có thể còn DB nhưng **ẩn** khỏi `ListMineAsync`. | `ChannelService.CreateAsync`, `ListMineAsync` |
...
| BR-088 | Mỗi org có **một** kênh `Organization`; tạo lúc **`POST /register`** (bootstrap org) và ensure khi list chat (org cũ); tên = tên org hoặc `"Toàn công ty"`. **Mọi nhân viên active** tự vào kênh; nhân viên mới tạo/provision vào ngay; **terminate** gỡ membership. | `AuthService.RegisterAsync`, `OrgChannelService`, `EmployeeService` |
| BR-089 | `ListMineAsync` chỉ trả **Organization + Direct**. `GET /channels/org/members` liệt kê NV active (tối đa 200) để nhắn riêng. `POST /channels` chỉ **Direct**, mọi role có `Employee` cùng org; tái sử dụng DM nếu đã có. | `ChannelService`, `ChannelEndpoints` |
...
| BR-072 | Auto-scheduling theo phòng ban/tuần. **Policy org** (`org-scheduling-policy.v1.1`) dùng chung mọi chi nhánh; catalog `GET /scheduling/rule-catalog`; Admin `GET/PUT /org/scheduling-policy`, wizard draft (Manager chỉ đọc). **Luật enforced mặc định `enabled: false`** — không bật thì CP-SAT chạy **pure preference** (có thể xếp quá nhiều NV vào một ca). Bật để kiểm soát chất lượng gợi ý: min/max người/ca (`default_min_staff_per_shift`, `default_max_staff_per_shift`), trần ca/ngày/tuần/NV, nghỉ giữa ca, đăng ký ca, khớp vai trò. PUT policy validate feasibility → **422** khi min > max hoặc xung đột staffing. | `CpSatScheduleSuggestionService`, `OrganizationSchedulingSolverPolicy`, `SchedulingRuleCatalog`, `OrganizationSchedulingPolicyWizard` |
...
| BR-086 | Gợi ý CP-SAT: NV đã **gửi** bảng đăng ký — chỉ ô **Ưu tiên/Có thể** được xếp; ô **Trống** (không có dòng) hoặc **Không** bị chặn. Phân ca nháp khóa theo từng NV/ô: khi một NV cập nhật đăng ký sau lần phân ca/apply gần nhất, chỉ phân ca nháp của chính NV đó được mở để re-suggest; NV khác vẫn khóa trừ khi họ cũng đổi đăng ký hoặc phân ca của họ trùng Unavailable. Áp dụng upsert đúng tuple `(shiftDefinitionId, employeeId, date)` và ghi `Schedule.SuggestionsAppliedAt`; nhiều NV được giữ trên cùng `(ca, ngày)` khi policy staffing cho phép, nên gợi ý của NV này không được ghi đè/xóa phân ca của NV khác. Khi `ClearOrphanAssignments=true`, chỉ xóa tuple bị omit của nhóm NV bị ảnh hưởng; nếu không bật thì phân ca bị omit được giữ nguyên. FE so sánh lịch nháp vs gợi ý mới trước Áp dụng. | `SchedulingAssignmentRules`, `SchedulingAssignmentLockPolicy`, `ScheduleRebalanceBaseline`, `CpSatScheduleSuggestionService`, `ApplySuggestionsAsync` |
...
| BR-097 | **Gợi ý lịch (tạm thời, chỉ một lần gọi):** Manager xác nhận một gợi ý có cấu trúc từ trợ lý insight để ảnh hưởng đúng một lần gọi `suggest` tới qua trường `Hint` tùy chọn trong `SuggestScheduleRequest`. **Ba hình dạng gợi ý:** (a) PreferenceWeight—tăng/giảm điểm cho role/phòng ban hoặc danh sách NV cụ thể; (b) TempMinMax—ghi đè giới hạn tổng số phân ca cho một nhóm/người chỉ trong lần gọi này; (c) AvoidPairing—ngăn hai NV cùng ô (ca + ngày) trong một lần gọi. Gợi ý **không bao giờ** lưu vào `OrganizationSchedulingPolicy` hay bảng policy nào; chỉ ảnh hưởng lần gọi `suggest` ngay lập tức. Gợi ý không hợp lệ (validate roster thất bại) → response trả `Reason: "invalid_hint"`; không khả thi → `Reason: "hint_infeasible"` (khác với `"infeasible"` chung). `apply-suggestions` **không có trường hint** — gợi ý không có đường ghi. Endpoint `/schedules/{id}/insights/chat` có thể sinh gợi ý qua toggle `HintMode` (Bedrock chỉ advisory; chat không bao giờ ghi phân ca). | `ScheduleInsightService.ChatAsync`, `ScheduleSuggestionHintValidator`, `CpSatScheduleSuggestionService.SolveAsync` |
