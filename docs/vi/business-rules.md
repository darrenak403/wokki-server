...
| BR-008 | Org Admin tạo staff qua `POST /api/v1/employees` kèm `DepartmentId`; một workflow này tạo cả tài khoản `User` và hồ sơ `Employee`, rồi tự tạo **Active** `LocationMembership` tại chi nhánh của phòng ban đó. Nếu đã có `User` legacy cùng org nhưng chưa có `Employee`, workflow này sẽ liên kết user đó thành hồ sơ nhân viên thay vì để tài khoản mồ côi. Không tạo tài khoản org staff trần qua `/users` khi chưa có `Employee`. Nhân viên đăng nhập vào `/app` trực tiếp — không có luồng tự gửi yêu cầu tham gia hay màn `/pending`. | `EmployeeService.CreateAsync`, `UserService.CreateAsync`, `EmployeeService.EnsureActiveLocationMembershipAsync` |
