...
## 4. Bảng tin đổi ca (Draft)

```mermaid
stateDiagram-v2
    [*] --> Pending: User đăng Cover/CrossSwap
    Pending --> Completed: Đồng nghiệp accept FCFS
    Pending --> Cancelled: Tác giả huỷ
    Pending --> Hidden: Lịch publish
    Pending --> Expired: Phân ca không còn hợp lệ
    Completed --> [*]
    Hidden --> [*]
    Cancelled --> [*]
    Expired --> [*]
...
### Accept (nguyên tử, FCFS)

```mermaid
sequenceDiagram
    participant B as Người nhận (User)
    ...
    alt Cover
        S->>DB: Chuyển EmployeeId phân ca
    else CrossSwap
        S->>DB: SwapEmployeeIdsAsync
    end
    S->>DB: post → Completed, audit log
    S->>DB: COMMIT
    S->>S: Email tác giả + người nhận (best-effort)
    API-->>B: 200
```

Publish khóa schedule và ẩn bài Pending trước khi chuyển `Published`.

**BR-030–BR-037**: xem [business-rules.md](./business-rules.md).
