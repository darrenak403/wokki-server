...
public interface IOvertimeRequestService
{
    ...
    Task<ApiResponse<PagedResponse<OvertimeRequestResponse>>> ListPendingAsync(Guid reviewerUserId, bool isAdmin, Guid? departmentId, int page, int pageSize, CancellationToken ct = default);
    Task<ApiResponse<PagedResponse<OvertimeRequestResponse>>> ListAllAsync(Guid reviewerUserId, bool isAdmin, Guid? departmentId, int month, int year, int page, int pageSize, CancellationToken ct = default);
